...
categories = %w(Epic Animals History Fantasy Adventure)
...
titles.each do |title|
	Video.create(title: title, description: "A film", small_cover_url: "#{small_base_url}+#{random_color}/000000?text=#{title.upcase.titleize}", large_cover_url: "#{large_base_url}" + title.upcase, category: Category.create(name: categories.sample))
end
